...
def solve_psi_W(param, Ny, Nz, degree=1):
    ...
    dWdn_negativetab = df.Constant(
        param.Ly / (param.alpha_prime * param.sigma_cn_dblprime * param.A_tab_n)
    )
    dWdn_positivetab = df.Constant(
        param.Ly / (param.alpha_prime * param.sigma_cp_dblprime * param.A_tab_p)
    )
    ...
    mixed_functionspace = df.FunctionSpace(mesh, lagrange_element * real_element)
    ...
    a_psi = (
        df.inner(df.grad(psi), df.grad(psi_test)) + c1 * psi_test + psi * c2
    ) * df.dx
    L_psi = dpsidn_negativetab * psi_test * ds(1) + dpsidn_positivetab * psi_test * ds(
        2
    )

    a_W = (df.inner(df.grad(W), df.grad(W_test)) + c3 * W_test + W * c4) * df.dx
    L_W = (
        -W_test * df.dx
    ...
    R_CC = (
        ...
        * (
        ...
        - (psi_tab_p - psi_tab_n)
    ) / (param.sigma_cn_dblprime * param.L_cn + param.sigma_cp_dblprime * param.L_cp)

    # Compute effective resistance for current collector heating R_cn and R_cn
    grad_phi_cn = (
        df.grad(psi)
        + (param.alpha_prime / param.Ly)
    ...
    R_cn = (param.sigma_cn_dblprime * param.L_cn / param.Ly) * df.assemble(
        df.inner(grad_phi_cn, grad_phi_cn) * df.dx
    )
    ...
    R_cp = (param.sigma_cp_dblprime * param.L_cp / param.Ly) * df.assemble(
