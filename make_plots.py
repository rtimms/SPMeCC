...
def plot_voltage_breakdown(soln, mesh, R_cc, param):
    ...
    t = soln.t * param.tau_d_star
    print(param.I_star)
    dCap = t * param.I_star / (60 * 60) / param.C_rate
    print(t)
    print(dCap)
    # Font stuff
    plt.rc("text", usetex=True)
    ...
    plt.xlabel(r"Discharge Capacity (Ah/m$^2$)", fontsize=18)
...
def plot_heat_generation(soln, mesh, R_cn, R_cp, param):
    ...
    plt.plot(
        t * param.tau_d_star, heat.rxn_n_0(T0, c_n_surf, param, I_app), label="rxn"
    )
    plt.plot(
        t * param.tau_d_star, heat.rev_n_0(T0, c_n_surf, param, I_app), label="rev"
    )
    plt.plot(
        t * param.tau_d_star,
    ...
    plt.plot(0, 0, label="Ohm")
    plt.plot(
        t * param.tau_d_star, heat.rxn_p_0(T0, c_p_surf, param, I_app), label="rxn"
    )
    plt.plot(
        t * param.tau_d_star, heat.rev_p_0(T0, c_p_surf, param, I_app), label="rev"
    )
    plt.plot(
        t * param.tau_d_star,
    ...
    plt.plot(
        t * param.tau_d_star, heat.rev_n_1(T1, c_n_surf, param, I_app), label="rev"
    )
    plt.plot(
        t * param.tau_d_star,
        heat.ohmic_n_1(c_e_n_bar, c_e_neg_sep, param, I_app)
    ...
    plt.plot(
        t * param.tau_d_star, heat.rev_p_1(T1, c_p_surf, param, I_app), label="rev"
    )
    plt.plot(
        t * param.tau_d_star,
        heat.ohmic_p_1(c_e_p_bar, c_e_pos_sep, param, I_app)
    ...
    if R_cn == 0:
        plt.subplot(2, 5, 1)
        plt.plot(
            t * param.tau_d_star, (I_app / param.Ly) ** 2 / param.sigma_cn, label="Ohm"
        )
        plt.xlabel(r"$t$ [s]", fontsize=11)
        ...
        plt.subplot(2, 5, 6)
        plt.plot(t * param.tau_d_star, heat.ohmic_cc_1(R_cn, param, I_app), label="Ohm")
        plt.xlabel(r"$t$ [s]", fontsize=11)
    ...
    if R_cp == 0:
        plt.subplot(2, 5, 5)
        plt.plot(
            t * param.tau_d_star, (I_app / param.Ly) ** 2 / param.sigma_cp, label="Ohm"
        )
        plt.xlabel(r"$t$ [s]", fontsize=11)
        ...
        plt.subplot(2, 5, 10)
        plt.plot(t * param.tau_d_star, heat.ohmic_cc_1(R_cp, param, I_app), label="Ohm")
        plt.xlabel(r"$t$ [s]", fontsize=11)
...
def plot_electrolyte_concentration(soln, mesh, param):
    ...
    c_e_n_steady = (
        ...
        * (
            ...
            * (
            ...
            + 3
            * param.L_s
            * (param.L_p - param.L_n + 1)
            / param.epsilon_s ** param.brug
            + 3
            * (param.L_n ** 2 - mesh.x_n ** 2)
            / param.L_n
    ...
    for i in range(1, np.size(soln.t)):
        ...
        plt.plot(
            mesh.x_n, param.c_e_typ_star * param.delta * c_e_n_steady * I_app, "r--"
        )
        plt.plot(
        ...
        plt.plot(
            mesh.x_s, param.c_e_typ_star * param.delta * c_e_s_steady * I_app, "r--"
        )
        plt.plot(
        ...
        plt.ylabel(
            r"$c_{{\mathrm{{e}}}}^* - c_{{\mathrm{{e,typ}}}}^*$" r"[mol/m$^3$]",
            fontsize=11,
        )
        plt.legend()
        fig.tight_layout()
...
def plot_psi_W(psi, W, R_CC, param):
    ...
    fig.suptitle(
        r"$\alpha^{{\prime}} = {:.3f}$. "
        "$R_{{\mathrm{{CC}}}}$ = {:.3f}".format(param.alpha_prime, R_CC),
        fontsize=16,
    )
    fig.tight_layout()
