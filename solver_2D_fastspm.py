...
dVdn_negativetab = Constant(-I_app/(param.sigma_cn_prime*param.A_tab_n))  # dV/dn at -ve tab
dVdn_positivetab = Constant(-I_app/(param.sigma_cp_prime*param.A_tab_p))  # dV/dn at +ve tab


# Timestepping ----------------------------------------------------------------
t = 0.0  # initial time
t_final = (15*60)/param.tau_d_star  # final time
...
def U_n(c, T, param):
    U_ref = (0.7222 + 0.1387 * c + 0.029 * c ** 0.5 - 0.0172 / c
             + 0.0019 / (c ** 1.5) + 0.2808 * exp(0.9 - 15 * c)
...
def eta_n(I, c, T, param):
    result = (2*((1 + param.Theta*T)/param.Lambda)
              * (ln(I/(g_n(c, param)*param.L_n)
...
def Q_bar(psi, V, I, c_n, c_p, T, param):
    phi_cn = (I_app*psi - param.sigma_cp_prime*param.L_cp*V)/(param.sigma_cn_prime*param.L_cn + param.sigma_cp_prime*param.L_cp)
    phi_cp = (I_app*psi + param.sigma_cn_prime*param.L_cn*V)/(param.sigma_cn_prime*param.L_cn + param.sigma_cp_prime*param.L_cp)
...
class NegativeTab(SubDomain):
    def inside(self, x, on_boundary):
        if param.tab_n_location == 't':
            return near(x[1], 1.0) and \
                between(x[0], (param.tab_n_centre - param.L_tab_n / 2,
            ...
            return near(x[0], 0.0) and \
                between(x[1], (param.tab_n_centre - param.L_tab_n / 2,
                                  param.tab_n_centre + param.L_tab_n / 2)
                           )
        elif param.tab_n_location == 'r':
            return near(x[0], param.Ly) and \
                between(x[1], (param.tab_n_centre - param.L_tab_n / 2,
                                  param.tab_n_centre + param.L_tab_n / 2)
                           )
        else:
...
class PositiveTab(SubDomain):
    def inside(self, x, on_boundary):
        if param.tab_p_location == 't':
            return near(x[1], 1.0) and \
                between(x[0], (param.tab_p_centre - param.L_tab_p / 2,
            ...
            return near(x[0], 0.0) and \
                between(x[1], (param.tab_p_centre - param.L_tab_p / 2,
                                  param.tab_p_centre + param.L_tab_p / 2)
                           )
...
u_mixedelement = MixedElement([u_element, u_element,
                               u_element, u_element, u_element])
...
while (t < t_final):
    ...
    F1 = (inner(grad(V), grad(V_test))
          - param.alpha*I*V_test)*dx \
        - dVdn_negativetab*V_test*ds(1) - dVdn_positivetab*V_test*ds(2)
    F2 = (V - (U_p(c_p, T, param) - U_n(c_n, T, param))
          - (eta_p(I, c_p, T, param) - eta_n(I, c_n, T, param))
          )*I_test*dx
    F3 = (c_n - c_n_prev
          + I*3*dt/(param.beta_n*param.C_hat_n*param.L_n))*c_n_test*dx
    F4 = (c_p - c_p_prev
          - I*3*dt/(param.beta_p*param.C_hat_p*param.L_p))*c_p_test*dx
    F5 = (param.rho / param.gamma_th)*(T - T_prev)*T_test*dx \
        ...
        + dt*(param.epsilon/param.L)*((param.h_tab_prime*(param.L_cn + param.L_cp))
                                           + param.h_prime)*T*T_test*ds(1) \
        + dt*(param.epsilon/param.L)*((param.h_tab_prime*(param.L_cn + param.L_cp))
                                           + param.h_prime)*T*T_test*ds(2)
    F = F1 + F2 + F3 + F4 + F5
    ...
    I_app_num = I_app*assemble((1/(param.sigma_cn_prime*param.A_tab_n))*ds(1)
                               + (1/(param.sigma_cp_prime*param.A_tab_p))*ds(2))/param.alpha
    I_error = np.linalg.norm(I_app_num - I_dx)
    print('Check current conservation: I_app = {:.2f}, I_app_num = {:.2f}, '
          'I*dx = {:.2f}, '
...
plt.title('Potential [V]', fontsize=18)
...
plt.xlabel(r'$y$', fontsize=18)
plt.ylabel(r'$x$', fontsize=18)
plt.title('Temperature [K]', fontsize=18)
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
